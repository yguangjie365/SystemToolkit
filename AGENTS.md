...
🚧 **音乐管理已交付（M5 + 在线流阶段二；MUSIC-7 已于 2026-09-12 验收通过——迷你播放条改版为独立迷你窗），下一站设置模块 + V1.0 重装助手**（已交付：概览/软件/驱动/网络/互传/游戏/备份/音乐；项目总控与路线图见 [`Docs/00-项目总控.md`](Docs/00-项目总控.md)）。
...
🔴 **多 Agent 协作三条铁律**（详见 [`Docs/09-开发规范/05`](Docs/09-开发规范/05-多Agent协作与提交规范.md)）：

1. **先认领后动手** —— 在 [`TASKS.md`](Docs/09-开发规范/TASKS.md) 署名改状态，未认领的改动可被回滚
...
> 技术栈已定：**WPF on .NET 10 LTS**。决策依据见 [ADR-001](Docs/decisions/ADR-001-UI技术栈选型.md)。
...
**唯一权威值表 = `Docs/04-UI设计规范.md` §三（自实现反向校准）**。
...
| 设计基准 | 1440×900（16:10）；最小窗口 1240×640（2026-09-04 由 1024 调整：保证软件管理页最低列宽配置不裁剪，依据见 [07-WPF界面实现经验](Docs/09-开发规范/07-WPF界面实现经验.md)） |
...
当前初始风格：**Claude 暖调**（奶油白 `#faf9f5` + 珊瑚橙 + 深海军蓝）。详见 [ADR-003](Docs/decisions/ADR-003-主题系统与初始视觉风格.md) 与 [`Docs/04-UI设计规范.md`](Docs/04-UI设计规范.md)。

🟠 **UI 实现前必读**：[`Docs/09-开发规范/07-WPF界面实现经验.md`](Docs/09-开发规范/07-WPF界面实现经验.md)——V0.2 沉淀的 DIP/物理像素换算、字号字重基线（雅黑无 Medium）、GridView 列宽自适应方法论、WPF 默认值陷阱清单。写 XAML 前不读此文档，会重复踩已付过学费的坑。
...
- 单个超长 UI 文件的写法（`MusicPage.razor` 169KB 是反模式，本项目文件上限见 [01 分册](Docs/09-开发规范/01-编码与命名规范.md#三文件组织)）
...
- **文档分工**：
  ...
  - `Docs/09-开发规范/` —— **开发规范唯一权威来源**
  - `Docs/Design/` —— 模块详细设计
  - `Docs/0X-*` —— 上游设计文档
- 🔴 **文档不得漂移**：写"已实现"前必须 grep 实码核实。旧工程曾出现文档声称 `BlazorTheme` 已持久化、实际代码并未实现的事故。
- 🟠 文档与代码同步更新，顺手清理陈旧信息。
- 🟠 设计文档未定稿前，任何修订需与用户确认。
- 🟠 改动影响对外行为时，同步清单见 [04 分册第三节](Docs/09-开发规范/04-文件与目录规范.md#三修改文件规范)。
- 🟡 审查与待办项先查索引文档，避免重复劳动。
