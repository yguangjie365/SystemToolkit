...
🚧 **音乐管理已交付（M5 + 在线流阶段二；MUSIC-7 于 2026-09-12 验收通过）；开源对标计划推进至 B11 全批完成（手机端对话式改造 W1a~W3c）· B10 完成（UI 令牌棘轮覆盖洞清扫）· B1 设计稿已交付待审（设置模块）→ 下一站 = 设置模块实现批次（待评审）+ V1.0 重装助手**（已交付：概览/软件/驱动/网络/互传/游戏/备份/音乐；项目总控与路线图见 [`Docs/00-入口/00-项目总控.md`](Docs/00-入口/00-项目总控.md)）。

- 需求/设计文档的**修订**仍需与用户沟通后执行（见 04-UI 设计规范 v2 的裁定记录）。
- 🔴 **上一个工程 `D:\MyProject\FileBackup.CSharp` 已于 2026-09-13 由用户删除**（用户裁决：除「设置」「重装助手」外 8 个模块均已交付，**旧工程已无任何参考意义**）。
  → **「从旧工程复用 / 主动翻找旧工程」类指令一律失效**（§八 已同步改写）。
  → 但文档里「旧工程教训 / 旧工程实测」类引用**保留**：它们是多条规则的**立规依据**，删掉就等于丢掉"为什么"。
...
## 八、旧工程资产处理（🔴 2026-09-13 起：旧工程已移除）

> 🔴 **状态变更（2026-09-13 用户裁决）**：旧工程 `D:\MyProject\FileBackup.CSharp`（283 `.cs` / 约 4.4 万行 / 474 测试 / WPF + BlazorWebView）**已被用户删除**。原因：除「设置」「重装助手」两个模块外，其余 8 个模块均已良好交付，**旧工程已无任何参考价值**。
>
> **由此失效的规则**（原文保留于下，仅作历史，**不再执行**）：
> - ~~「功能层默认可复用 / 可复用的直接复用、禁止重写」~~ —— **已无可复用来源**
> - ~~「每个模块开工前主动翻找旧项目对应域实现（含埋在弃用 UI 里的纯逻辑）」~~ —— **路径不存在，勿再尝试**
> - `Docs/40-开发规范/04-文件与目录规范.md` 的「不修改旧工程文件」—— 标的物已不存在
>
> **仍然有效**：
> ① 本节 ❌ **一律不复用** 清单——它解释的是**技术判断**（`Wpf.Ui` 隐式样式渗透、BlazorWebView 管线、超长 UI 文件反模式），不是"依赖旧工程存在"；
> ② 🟠 第三方代码引入的许可证流程（ADR-002 §5）——通用纪律，与来源无关；
> ③ 文中所有「旧工程教训 / 旧工程实测」引用——它们是**多条规则的立规依据**，删掉就等于丢掉"为什么"。

### 🔴 UI 先审后写（用户 2026-09-04 补充 —— **独立纪律，与旧工程无关，持续有效**）

任何功能页面的 UI 代码开发前，必须先向用户**描述布局方案并出示意图**，**经用户确认后**才能写 UI 代码。包括 XAML 结构、区域划分、交互要素；确认记录留在任务/变更记录中。

### 历史记录：旧工程迁移清单（**仅供追溯，不再是行动指引**）

下列资产依上述纪律已于 2026-09 从旧工程迁入本项目。清单保留**仅为追溯某段代码的出处与迁移时的取舍**（如 ATL → TagLibSharp 的许可证替换、`MusicService` 拆分）。
🔴 **新模块不必再去找它们——本项目内的现有实现才是权威。**
...
### ⚠️ UI 层（历史）：旧工程仅两页曾被判定可参考

> 🔴 **2026-09-13：本节已失效**——旧工程及其 razor 文件已删除，下述参照物**不可访问**。记录保留以便追溯"为何本项目概览页与游戏页的布局走这个方向"。

用户曾明确：**原项目"本机预览"与"游戏管理"这两个页面还可以，其它凌乱不堪。**
